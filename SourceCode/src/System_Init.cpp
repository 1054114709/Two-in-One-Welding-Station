...
void System_Init()
{
    ...
    if (ui_InitializeStatusBar)
        lv_bar_set_value(ui_InitializeStatusBar, 8, LV_ANIM_ON);
    if (ui_InitializeStatus)
        lv_textarea_add_text(ui_InitializeStatus, "显示屏初始化成功.\n");
    if (ui_InitializeStatusBar)
        lv_bar_set_value(ui_InitializeStatusBar, 16, LV_ANIM_ON);
    if (ui_InitializeStatus)
        lv_textarea_add_text(ui_InitializeStatus, "GPIO初始化成功.\n");
    ...
    if (ui_InitializeStatus) {
        sprintf(status_buffer, "INA226初始化: %s.\n", ina226_ok ? "成功" : "错误");
        lv_textarea_add_text(ui_InitializeStatus, status_buffer);
    }
    
    if (!ina226_ok) {
        handle_init_failure("错误: INA226初始化错误!\n");
        return; // 停止初始化
    }
    lvgl_friendly_delay(INIT_STEP_DELAY_MS);
    ...
    if (!tmp102_ok) {
        handle_init_failure("错误: TMP102初始化错误!\n");
        return; // 停止初始化
    }
    lvgl_friendly_delay(INIT_STEP_DELAY_MS);
    ...
    if (ui_InitializeStatus) lv_textarea_add_text(ui_InitializeStatus, "MAX6675初始化成功.\n");
    lvgl_friendly_delay(INIT_STEP_DELAY_MS);
    ...
    if (ui_InitializeStatus) {
        sprintf(status_buffer, "电烙铁热电偶: %s (状态:%d).\n", soldering_ok ? "正常" : "错误", soldering_status);
        lv_textarea_add_text(ui_InitializeStatus, status_buffer);
    }
    
    if (!soldering_ok) {
        handle_init_failure("错误: 烙铁热电偶检测错误!\n");
        return; // 停止初始化
    }
    lvgl_friendly_delay(INIT_STEP_DELAY_MS);
    ...
    if (ui_InitializeStatus) {
        sprintf(status_buffer, "热风枪热电偶: %s (状态:%d).\n", heatgun_ok ? "正常" : "错误", heatgun_status);
        lv_textarea_add_text(ui_InitializeStatus, status_buffer);
    }
    
    if (!heatgun_ok) {
        handle_init_failure("错误: 热风枪热电偶检测错误!\n");
        return; // 停止初始化
    }
    lvgl_friendly_delay(INIT_STEP_DELAY_MS);
    ...
    if (ui_InitializeStatus) lv_textarea_add_text(ui_InitializeStatus, "电烙铁PID初始化成功.\n");
    lvgl_friendly_delay(INIT_STEP_DELAY_MS);
    ...
    if (ui_InitializeStatus) lv_textarea_add_text(ui_InitializeStatus, "热风枪PID初始化成功.\n");
    lvgl_friendly_delay(INIT_STEP_DELAY_MS);
